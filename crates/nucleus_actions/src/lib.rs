...
pub mod agents_sidebar {
    ...
    actions!(
        ...
        [
            ...
            FocusSidebarFilter,
            /// Opens the Chat Clusters grid — organizational containers for
            /// grouping threads. Dispatched from the status bar's browse-
            /// threads button; handled in `agent_ui` (registered against
            /// `Workspace` so `workspace` itself never depends on
            /// `agent_ui`, same indirection `ToggleThreadSwitcher` uses).
            ToggleChatClusters,
        ]
    );
}
