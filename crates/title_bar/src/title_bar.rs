    ...
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        children.push(
            h_flex()
                ...
                .child(self.render_theme_toggle_button(cx))
...
impl TitleBar {
    ...
    pub fn render_theme_toggle_button(&mut self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        .tab_index(0isize)
        .aria_label("Toggle Nucleus Light/Dark Theme")
        .tooltip(Tooltip::text(if is_dark {
            "Switch to Nucleus White"
        } else {
            "Switch to Nucleus Black"
        }))
        .on_click(move |_, window, cx| {
            window.dispatch_action(zed_actions::theme::ToggleMode.boxed_clone(), cx);
        })
    }
